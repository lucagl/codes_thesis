...
use com_var
implicit none
logical, private :: iflast, check!,check_integration
logical, save :: read_last!,read_check_conc
integer, private ::  initial_generator
character (len=30) :: read_name,read_lenght,read_concentration,read_load
...
CONTAINS
	...
	subroutine read_input_file()
		...
		inquire(file = run_name//'.lc',EXIST = check)
			...
			read(1,*) c_inf
			write(read_concentration,'(F0.2)') c_inf
			call get_conc_name(read_concentration)
		go to 10
		else
...
subroutine get_conc_name(input_string)
...
allocate(character(len=LEN(trim(input_string))+5):: concentration)

concentration= trim('cinf_'//trim(input_string))

end subroutine get_conc_name
