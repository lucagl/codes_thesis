...
&'Mean concetration =', mean_conc, 'Conc. in L(last bin)',exp(mu(M))+ nui*growth_vel(M) 
...
!		frame = trim(number)
		...
		write(104,*) -(L+delta_BC),(c0*exp(mu_BC)+nui*growthVel_BC)
		do j=1,2*M-1	
...
!-------------------------------------
    ...
    subroutine write_init()
		...
		write (20,*) '# t				z0			zMax2ndDer		zMin2ndDer			zMax2ndDerInt		zMin1stDerInt'
		
		open (15,file = 'crystal_pressure_alt2'//ext_force//supersaturation//'.out',action = 'write')
	...
	double precision, dimension (M) :: curv,der
	...
	open(unit = 3,file = 'final_2ndDer_interaction.out', action = 'write') 

	curv(1) = 1.0/3.0*(0.5d0*force(z(3))+7.5d0*force(z(1))-8.0d0*force(z(2)))*dri2
	write (3,*) 0, curv(1)
	do j = 2, M-1
		curv(j) = -(force(z(j+1))+force(z(j-1))-2.0d0*force(z(j)))*dri2
		write (3,*) (j-1)*deltax, curv(j)
	enddo
	curv(M) =  -(force(z(M-2))-2*force(z(M-1))+force(z(M)))*dri2
	write (3,*) L, curv(M)
	close(3)
...
write (3,*) 0, der(1)
do j = 2, M-1
der(j) = 0.5*(force(z(j+1))-force(z(j-1)))*dri
write (3,*) (j-1)*deltax, der(j)
enddo
der(M) = (force(z(M))-force(z(M-1)))*dri
write (3,*) L, der(M)
close(3)
...
close(3)
	...
	double precision, dimension (M) :: secDer_f,secDer,der
	double precision :: L_contact,L_contact_2,L_contact_3
	integer :: index_force,index_profileMx,index_profilemn,index_1derF
	
	integer j

	secDer(1) = 1.0/3.0*(0.5d0*z(3)+7.5d0*z(1)-8.0d0*z(2))*dri2
	do j = 2, M-1
		secDer(j) = -(z(j+1)+z(j-1)-2.0d0*z(j))*dri2
	enddo
...
!index = (minloc(secDer,1)-1)
	index_1derF = minloc(der,1)-1
	index_force = minloc(secDer_f,1)-1
	index_profileMx = maxloc(secDer,1)-1
	index_profilemn = minloc(secDer,1)-1


	L_contact = index_force*deltax
	L_contact_2 = index_profileMx*deltax
	L_contact_3 = index_1derF * deltax

	
	write(20,*) s*deltat,hs-z(1),hs-z(index_profileMx),hs-z(index_profilemn),hs-z(index_force),hs-z(index_1derF)
	...
	write(15,*)	s*deltat,L_contact_3,crystal_force/((L_contact_3)*(L_contact_3)*PI)
